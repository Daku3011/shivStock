    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 relative">
        ...
        <div className="flex items-center space-x-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Remove Stock (Outward)</h3>
            <p className="text-xs text-slate-400">
              {item.name} &bull; <span className="font-mono text-rose-400">{item.sku}</span>
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-slate-300">Quantity to Dispatch (Sheets)</label>
              <span className="text-xs text-slate-400">
                Available: <strong className="text-white">{item.quantity}</strong> &rarr; Remaining:{' '}
                <strong
                  className={
            ...
            <input
              ...
              className="w-full h-12 px-4 rounded-xl bg-slate-950 border border-slate-700 text-white text-lg font-bold focus:border-rose-500 focus:ring-1 focus:ring-rose-500 outline-none transition-all"
              required
            />
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-11 px-4 rounded-xl bg-slate-950 border border-slate-700 text-sm text-white focus:border-rose-500 outline-none"
            />
          </div>

          <div>
            <label className="text-xs font-semibold text-slate-300 block mb-1.5">Reason / Dispatch Note</label>
            <input
              type="text"
              value={reason}
          ...
          {quantity > 0 && item.quantity - quantity <= item.min_threshold && (
            <div className="flex items-center space-x-2 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs">
          ...
          <div className="pt-2 flex items-center space-x-3">
            ...
            <button
              ...
              className="flex-1 h-11 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-sm font-bold shadow-lg shadow-rose-600/30 flex items-center justify-center space-x-2 transition-all disabled:opacity-50"
            >
